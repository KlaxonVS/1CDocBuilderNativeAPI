...
typedef std::variant<std::monostate, int32_t, double, bool, std::string,
                     std::tm, std::vector<char> >
    variant_t;
...
class Component : public IComponentBase {
 public:
  ...
  const WCHAR_T *ADDIN_API GetMethodName(const long num,
                                         const long lang_alias) final;
  ...
  bool ADDIN_API GetParamDefValue(const long method_num, const long param_num,
                                  tVariant *def_value) final;

  bool ADDIN_API HasRetVal(const long method_num) final;

  bool ADDIN_API CallAsProc(const long method_num, tVariant *params,
                            const long array_size) final;

  bool ADDIN_API CallAsFunc(const long method_num, tVariant *ret_value,
                            tVariant *params, const long array_size) final;

 protected:
  virtual std::string extensionName() = 0;

  void AddError(unsigned short code, const std::string &src,
                const std::string &msg, bool throw_excp);

  bool ExternalEvent(const std::string &src, const std::string &msg,
  ...
  void AddMethod(const std::wstring &alias, const std::wstring &alias_ru, C *c,
 ...
 private:
  ...
  static auto refTupleGen(std::vector<variant_t> &v,
...
template <size_t... Indices>
auto Component::refTupleGen(std::vector<variant_t> &v,
                            std::index_sequence<Indices...>) {
  return std::forward_as_tuple(v[Indices]...);
}

template <typename T, typename C, typename... Ts>
void Component::AddMethod(const std::wstring &alias,
                          const std::wstring &alias_ru, C *c, T (C::*f)(Ts...),
  ...
  MethodMeta meta{
      ...
      [f, c](std::vector<variant_t> &params) -> variant_t {
        auto args =
            refTupleGen(params, std::make_index_sequence<sizeof...(Ts)>());
