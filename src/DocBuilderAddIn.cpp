...
void DocBuilderAddIn::fillRow(const variant_t &range, const variant_t &rowData) {
    if (!WorkDirIsSet || !FileIsSet) {
        return;
    }
    unsigned int extension = getExtension(std::get<std::string>(*pathToFile));
    if (extension == 0 || extension < static_cast<unsigned int>(ExtensionUINT::XLSX) || extension > static_cast<unsigned int>(ExtensionUINT::OTS)) {
        AddError(
            ADDIN_E_FAIL, extensionName(),
            u8"Не поддерживаемое расширение исходного документа.",
            false
    ...
    if (!std::holds_alternative<std::string>(range) || !checkRangeInOneRow(std::get<std::string>(range))) {
        AddError(
            ADDIN_E_FAIL, extensionName(),
            u8"Диапазон ячеек должен быть в одной строке.",
            false
        );
        return;
    }
    ...
    std::vector<std::string> colDataArr = splitString(std::get<std::string>(rowData), ";");
    ...
    oWorksheet.Call("GetRange", NSDoctRenderer::CDocBuilderValue(sRange.c_str())).Call("SetValue", rArray);
}
...
bool DocBuilderAddIn::checkRangeInOneRow(const std::string range) {
    ...
    while (std::regex_search(rangeCpy, matches, pattern)) {
        ...
        rangeCpy = matches.suffix(); // continue searching from the end of the previous match
        count++;
        if (count == 2) {
            break; 
        }
