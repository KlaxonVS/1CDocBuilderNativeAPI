...
long Component::GetNProps() { return properties_meta.size(); }
...
const WCHAR_T *Component::GetPropName(long num, long lang_alias) {
  ...
  storeVariable(
      std::u16string(reinterpret_cast<const char16_t *>(name->c_str())),
      &result);
...
long Component::GetNMethods() { return methods_meta.size(); }
...
bool Component::GetParamDefValue(const long method_num, const long param_num,
                                 tVariant *def_value) {
...
bool Component::CallAsProc(const long method_num, tVariant *params,
...
bool Component::CallAsFunc(const long method_num, tVariant *ret_value,
                           tVariant *params, const long array_size) {
  try {
    auto args = parseParams(params, array_size);
...
void Component::AddError(unsigned short code, const std::string &src,
                         const std::string &msg, bool throw_excp) {
...
bool Component::ExternalEvent(const std::string &src, const std::string &msg,
...
void Component::AddProperty(
    const std::wstring &alias, const std::wstring &alias_ru,
...
void Component::AddProperty(const std::wstring &alias,
                            const std::wstring &alias_ru,
...
void Component::storeVariable(const variant_t &src, tVariant &dst) {
  ...
  if ((dst.vt == VTYPE_PSTR || dst.vt == VTYPE_BLOB) &&
      dst.pstrVal != nullptr) {
    memory_manager->FreeMemory(reinterpret_cast<void **>(&dst.pstrVal));
  }

  std::visit(
      overloaded{[&](std::monostate) { dst.vt = VTYPE_EMPTY; },
                 [&](const int32_t &v) {
                   dst.vt = VTYPE_I4;
                   dst.lVal = v;
                 },
...
void Component::storeVariable(const std::u16string &src, WCHAR_T **dst) {
  ...
  if (!memory_manager ||
      !memory_manager->AllocMemory(reinterpret_cast<void **>(dst), c_size)) {
    throw std::bad_alloc();
  };
...
void Component::storeVariable(const std::vector<char> &src, tVariant &dst) {
  ...
  if (!memory_manager ||
      !memory_manager->AllocMemory(reinterpret_cast<void **>(&dst.pstrVal),
                                   src.size())) {
    throw std::bad_alloc();
  };
...
std::vector<variant_t> Component::parseParams(tVariant *params,
                                              long array_size) {
...
std::string Component::toUTF8String(std::basic_string_view<WCHAR_T> src) {
  ...
  static std::wstring_convert<std::codecvt_utf8_utf16<char16_t>, char16_t>
      cvt_utf8_utf16;
  return cvt_utf8_utf16.to_bytes(
...
std::u16string Component::toUTF16String(std::string_view src) {
  ...
  static std::wstring_convert<std::codecvt_utf8_utf16<wchar_t>> cvt_utf8_utf16;
  std::wstring tmp =
      cvt_utf8_utf16.from_bytes(src.data(), src.data() + src.size());
  return std::u16string(reinterpret_cast<const char16_t *>(tmp.data()),
                        tmp.size());
#else
  static std::wstring_convert<std::codecvt_utf8_utf16<char16_t>, char16_t>
      cvt_utf8_utf16;
